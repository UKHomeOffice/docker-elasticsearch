...
export UNICAST_HOSTS_FILE=${UNICAST_HOSTS_FILE:=/hosts/unicast_hosts.txt}

[ -f $UNICAST_HOSTS_FILE ] && cp $UNICAST_HOSTS_FILE /elasticsearch/config/unicast_hosts.txt
...
export DISCOVERY_TYPE=${DISCOVERY_TYPE:=file}
...
export THREAD_POOL_QUEUE_SIZE=${THREAD_POOL_QUEUE_SIZE:=50}
